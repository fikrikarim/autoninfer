...
| 2026-08-18 19:55 | **MTP k=4 draft window: ratify adopting as canonical? (M1 +23.2%, near-tie-class gate).** Measured 2026-08-18 19:30-19:40 on the current engine (committed-source worktree binary at 18198e78, GPU 1, fixed M1 menu tg128 -r 3, same session): k=4 = **140.36 +/- 0.53 tok/s, 33.8% accept, 165 rounds** vs k=3 = 113.93 +/- 0.26 / 30.15% / 201 rounds = **+23.2%**; k=5 = 138.33 +/- 0.17 / 28.96% / 159 rounds = +21.4% (k=4 wins; k=2/k=3 were A/B'd earlier: k=3 kept at 117.35 vs 118.16). Mechanism: verify width 5 leaves the T<=4 GEMV bit-clone route (the bit-exact-verify cost that moved M1 130.13 -> 113.0) for the fast wide route, and the wide route's column hidden states improve draft quality (tg128 per-position accept: pos1 53.7% -> 67.3%, pos2 23.9% -> 41.8%). Quality gate (8 fixed prompts, greedy, 256 tok): k=4 vs k=3 = **8/8 prompts differ** - the same near-tie class as the ratified k=3 status quo (k=3 vs k=0 = 5/8, ratified 0ab130b8): token lengths match within ~5 tok (256/256, 224/225, 193/190, 244/256), no loops or cut-offs; haiku + code-c-bug spot check: word-level different reasoning that is semantically identical, both reach the correct answer. Per quality policy rule 4 (diff >= 3/8, speedup > 20%): measurement kept and logged; **ask: yes/no on adopting k=4 as the canonical draft window** (on yes: serve wrappers `--draft-tokens 4`, M1 menu + baseline -> 140.4, gate canonical MTP4; the loop flips the config in the next iteration). Companion to the 17:45 row: the near-tie class now buys +23% speed, not just lossless status quo. | Say yes/no. Until then k=3 remains canonical (config unchanged). | open since 19:55; 19:56 addendum: k=4 full-head draft also A/B'd = 129.28 tok/s (8% SLOWER than lm-head 140.36, gate 8/8 per-prompt hashes token-identical) -> if k=4 is ratified, **lm-head stays the head** (shortlist GEMV is the speed win at k=4); no separate decision needed |
| 2026-08-18 22:45 | **Request: Qwen3.8-27B BF16 source checkpoint + ~60-80 GiB disk, to enable the NVFP4 (Q4G64/Q5G64) text-weight re-quantization experiment.** Prefill audit (22:40, results row @ 902ff970): the 1.34x prefill lag vs qwen3.6-27b nvfp4 (8,340 vs 11,191 tok/s @7,680; local engine 9,186) is NOT a dispatch inefficiency — at the 1024-token prefill chunk the engine already takes the A8 tensor-core path (FP8 linear add 353/373 TFLOP/s = 84/89% peak; fused swiglu 432 TFLOP/s = 103% peak; 10-34x faster than A16). Root cause: the qwen3.8-27b/nvfp4 artifact carries its TEXT weights in 8-bit `FP8_E4M3FN_ROW_BF16S` (the converter "preserves source FP8 words" — the released checkpoint is FP8), while qwen3.6-27b/nvfp4 carries 4/5/6-bit `Q4G64/Q5G64/Q6G64` text weights; GEMMs are ~70-80% of prefill time, and 4/5-bit TC rates + half the weight bytes explain the gap. Re-quantizing qwen3.8 text weights from BF16 to the per-role Q4/Q5 recipe (tools/convert/qwen3_8_27b/convert_nvfp4.py exists; recipe extension + quality gate + fit check needed) is a checkpoint-content experiment in the user-authorized class (2026-08-18 rule) and would also speed DECODE (M1) via the W4A4 routes — expected the largest single lever left on M1/M2. **Not urgent**: the loop continues on other backlog items meanwhile. | (1) Make the Qwen3.8-27B BF16 checkpoint available locally (not on disk today; `models/` holds only the 21 GiB nvfp4 artifact) and ~60-80 GiB transient disk (26 GiB free today) OR confirm the FP8 checkpoint is the only source (in which case this experiment is dead-end — say so and I close it); (2) ratify/decline the re-quantization as a product-identity change after the loop measures it (gate + M1/M2; the qwen3.6 nvfp4 artifact is the existence proof for the format family at this scale). | open since 22:45 |
