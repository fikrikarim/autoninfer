...
**Last updated:** 2026-08-18 13:20 UTC by unattended driver session (iteration 27) — **backed off (20th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `a2a2d5f7` (iteration 26's backoff docs commit); this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:19):
  - **pid 21662** (pts/6, up 04:56, 27:21 CPU — flat, idle) and **pid 237820** (pts/5, up 24:21, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:19) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–27; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:19: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09.132, `fp8_linear_add_small_t.cu`/`fp8_linear_swiglu_plan.cpp` 12:54:09.136, `test_fp8_linear_t_parity.cpp` 12:06:23, `test_tmp_binary_diff.cpp` 12:47:39).

## Fresh /tmp evidence (read at 13:19, supersedes iteration 26's)
- **Last interactive write anywhere: still 12:54:09.** The `lh_*_iter26`/`iter27` entries (13:18–13:19) are driver scratch, not interactive writes.
- **Layer-hash dump state, re-counted this iteration (regex-extract `LH run=(\d+) (mix|mlp) layer=(\d+) h=([0-9a-f]+)` from `lh_old.err` and `lh_new.err` (both 52968 B, mtimes 12:52/12:54), paste-join in file order, compare 5-field halves):** both dumps 1152 tuples; **1088 differing, 64 matching; first differing tuple: 0-indexed position 8 = run=150 mix layer=4** (old h=`478a57cad0f3a017` vs new h=`4a44cf19f097a851`). Unchanged since iteration 15. The WIP FP8 fix was **not verified clean** at completion, unchanged since 12:54:09.
...
- The live harness serve is the **manual** process (**pid 21537, up 04:59 at 13:19**) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED (since 08:30); `ninfer-serve-standby` STOPPED (since 08:44). `/v1/models` → 200 at 13:19.
- `/tmp/autoninfer-ops/pending.json` still holds `{"action":"restart-primary"}` (queued 08:45, deferred while interactive sessions are alive; re-verified 13:19). Expect the **supervisor** serve (wrapper flags incl. `--pending-timeout-ms 300000`) to come up once no interactive session is alive. Verify at start: `supervisorctl status ninfer-serve` + `curl -s http://127.0.0.1:8080/v1/models`. Do NOT re-queue it.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:19: 1088/1152 layer-hash tuples still differ, first at line 9 = run=150 mix layer=4; no new interactive writes since 12:54:09).
...
- GPU 1: `bash tools/gpu_health.sh 1` before any benchmark. No GPU work ran this iteration (backoff #20).
