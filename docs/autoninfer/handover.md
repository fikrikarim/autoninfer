...
**Last updated:** 2026-08-18 13:08 UTC by unattended driver session `autoninfer-driver-16` — **backed off (9th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `4cfd0628` (iteration 15's backoff docs commit), pushed; this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:07):
  - **pid 21662** (pts/6, up 04:44, 27:21 CPU — unchanged since 13:05, i.e. idle) and **pid 237820** (pts/5, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:07) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–15; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:07: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09, `test_fp8_linear_t_parity.cpp` 12:06).

## Fresh /tmp evidence (read at 13:07, supersedes iteration 15's)
...
- **Layer-hash dump state, re-measured this iteration with the file-order (run,part,layer) join** (format `LH run=N mix|mlp layer=M h=hash`, embedded mid-line in the .err files; both dumps have 1152 tuples, runs 150–158, key sequences identical in file order, 0 key mismatches): **1088 differing, 64 matching. First differing tuple: 9th line = 0-indexed position 8 = run=150 mix layer=4** (layers 0–3 of run 150 match for both mix and mlp — lines 1–8 verified by hand). Unchanged from iteration 15's count. The WIP FP8 fix was **not verified clean** at completion, unchanged since 12:54:09.
- **GPU 0 util is NOT a clean interactive-activity signal:** the primary serve powers the interactive sessions AND the driver's own session (me). 0% at 13:06 (between my turns), 100% at 13:08 (during my turn) — attribution impossible.

## Serve state (do not touch)
- The live harness serve is the **manual** process (**pid 21537, started 08:20**) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED. `/v1/models` → 200 at 13:07.
- `/tmp/autoninfer-ops/pending.json` still holds `{"action":"restart-primary"}` (queued 08:45, deferred while interactive sessions are alive; re-verified 13:07). Expect the **supervisor** serve (wrapper flags incl. `--pending-timeout-ms 300000`) to come up once no interactive session is alive. Verify at start: `supervisorctl status ninfer-serve` + `curl -s http://127.0.0.1:8080/v1/models`. Do NOT re-queue it.
- GPU 1 idle (0%, 4 MiB) at 13:08. No GPU work ran this iteration (backoff), so no fresh `tools/gpu_health.sh 1` stamp.
...
1. **Check whether the interactive sessions are alive:** `ps -o pid,etime,cputime,stat -p 21662,237820` (plus `ps aux | grep -w "pi"` for any new one — exclude the driver's own `pi -p ... autoninfer-driver-N` process), then `git status --short`, `git log --oneline -3`, and `git diff | sha256sum` (expect `d49b4f84…bb705` if the WIP is untouched).
   - **Any interactive pi alive with a dirty tree → back off again exactly as iterations 8–16 did** (refresh this handover from fresh `/tmp` evidence — last-write scan `ls -lat /tmp | head -15`, `stat` the WIP files, re-count layer-hash diffs: extract `grep -oE 'LH run=[0-9]+ (mix|mlp) layer=[0-9]+ h=[0-9a-f]+' lh_old.err` / same for `lh_new.err`, paste in file order, diff the h= fields — 1088/64, first diff at 9th line = run=150 mix layer=4 is the current expectation — commit ONLY the handover + BLOCKERS state if it moved, push, end). Do not touch the tree.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:07: 1088/1152 layer-hash tuples still differ, first at 9th line = run=150 mix layer=4; no new writes since 12:54:08).
