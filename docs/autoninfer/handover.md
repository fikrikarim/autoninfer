...
**Last updated:** 2026-08-18 13:20 UTC by unattended driver session (iteration 26) — **backed off (19th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `71152f96` (iteration 25's backoff docs commit); this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:18):
  - **pid 21662** (pts/6, up 04:55, 27:21 CPU — flat, idle) and **pid 237820** (pts/5, up 23:24, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:18) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–26; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:18: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09.132, `fp8_linear_add_small_t.cu`/`fp8_linear_swiglu_plan.cpp` 12:54:09.136, `test_fp8_linear_t_parity.cpp` 12:06:23, `test_tmp_binary_diff.cpp` 12:47:39).

## Fresh /tmp evidence (read at 13:18, supersedes iteration 25's)
- **Last interactive write anywhere: still 12:54:09.** The `lh_*_iter25` entries (13:16–13:17) are iteration 25's own scratch (layer-hash recount), not interactive writes. This iteration's recount was in /tmp scratch as before (`lh_*_iter26` files, no writes to the WIP).
...
- (Scratch note: this iteration's first recount pass had a broken pipeline — it compared the new dump against itself and printed 1152/1152 same; the corrected paste-join recount above is the valid one.)

## Budget note
Driver budget is 30 h wall clock from ~09:00 UTC (2026-08-18) → clean exit around ~15:00 UTC (~1 h 40 min remaining at 13:20). If the interactive sessions stay alive, remaining iterations continue to back off in docs-only mode (that is by design; the driver stops cleanly on budget, and `supervisorctl start autoninfer-driver` re-arms it if the user wants more time).

## Serve state (do not touch)
- The live harness serve is the **manual** process (**pid 21537, up 04:58 at 13:18**) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED (since 08:30); `ninfer-serve-standby` STOPPED (since 08:44). `/v1/models` → 200 at 13:18.
...
1. **Check whether the interactive sessions are alive:** `ps -o pid,etime,cputime,stat -p 21662,237820` (plus `ps aux | grep -w "pi"` — exclude the driver's own session via the parent chain: its grandparent is `drive.sh` under `supervisord`), then `git status --short`, `git log --oneline -3`, and `git diff | sha256sum` (expect `d49b4f84…bb705` if the WIP is untouched).
   - **Any interactive pi alive with a dirty tree → back off again exactly as iterations 8–26 did** (refresh this handover from fresh `/tmp` evidence — last-write scan `ls -lat /tmp | head -15`, `stat` the WIP files, re-count layer-hash diffs: extract `grep -oE 'LH run=[0-9]+ (mix|mlp) layer=[0-9]+ h=[0-9a-f]+' lh_old.err` / same for `lh_new.err`, then `paste lh_old.iterNN.txt lh_new.iterNN.txt` and compare the two 5-field lines per row (fields 1–5 = old, 6–10 = new) — 1088/64 with first diff at 0-indexed 8 (line 9) = run=150 mix layer=4 (old `478a57cad0f3a017` vs new `4a44cf19f097a851`) is the current expectation — commit ONLY the handover + BLOCKERS state if it moved, push, end). Do not touch the tree. Remember: driver scratch files in /tmp (e.g. `lh_*_keys.txt`, `lh_diff.iterNN.txt`, `lh_*.iterNN.txt`) are not interactive writes; check mtimes against 12:54:09.
...
- GPU 1: `bash tools/gpu_health.sh 1` before any benchmark. No GPU work ran this iteration (backoff #19).
