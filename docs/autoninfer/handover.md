...
**Last updated:** 2026-08-18 ~13:05 UTC by unattended driver session `autoninfer-driver-12` — **backed off (5th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `5814e90a` (iteration 11's backoff docs commit on top of `83281ada`), pushed.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:01):
  - **pid 21662** (pts/6, up 04:38:46, 27:21 CPU — unchanged since the 13:00 sample of iter 11, i.e. idle) and **pid 237820** (pts/5, started 12:55:16, 0:02 CPU — idle) are both alive.
  ...
  - **WIP diff fingerprint (13:05) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 files, +105/−41); `tests/ops/test_fp8_linear_t_parity.cpp` sha256 = `92816bda…35748c`; `tests/ops/test_tmp_binary_diff.cpp` sha256 = `896bf98c…e12c13`.
...
## Fresh /tmp evidence (read at 13:05, supersedes iteration 11's)
- **Last interactive write anywhere: still 12:54:09** — `lh_old.err`/`lh_old.out` 12:54, `lh_new.err`/`lh_new.out` 12:52, `opdump_*.txt` 12:47–12:49, `m_new_*.err` 12:46, `ninfer_new2` 12:46. Nothing in the tree or /tmp has changed since (checked 13:01–13:05; tree mtimes 12:54:09, `text_context_impl.h` 12:50:36).
...
- GPU 1 idle (0%, 4 MiB) at 13:01. Last HEALTHY stamp 12:57:22 (triad 1466 GiB/s, FMA 110.7 TFLOP/s); re-verify before any benchmark per protocol. No GPU work ran this iteration (backoff).
...
1. **Check whether the interactive sessions are alive:** `ps -o pid,etime,cputime -p 21662,237820` (plus `ps aux | grep -w " pi"` for any new one), then `git status --short`, `git log --oneline -3`, and `git diff | sha256sum` (expect `d49b4f84…bb705` if the WIP is untouched).
   - **Any interactive pi alive with a dirty tree → back off again exactly as iterations 8–12 did** (refresh this handover from fresh `/tmp` evidence — last-write scan `ls -lat /tmp | head`, `stat` the WIP files — commit ONLY the handover + BLOCKERS state if it moved, push, end). Do not touch the tree.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:05: finished dumps still show ~1095 layer-hash diffs; no new writes since 12:54:09).
