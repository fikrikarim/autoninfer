...
**Last updated:** 2026-08-18 13:17 UTC by unattended driver session (iteration 24) — **backed off (17th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `1bbde703` (iteration 23's backoff docs commit); this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:16):
  - **pid 21662** (pts/6, up 04:53:38, 27:21 CPU — flat, idle) and **pid 237820** (pts/5, up 21:16, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:16) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–24; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:16: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09.132, `fp8_linear_add_small_t.cu`/`fp8_linear_swiglu_plan.cpp` 12:54:09.136, `test_fp8_linear_t_parity.cpp` 12:06:23, `test_tmp_binary_diff.cpp` 12:47:39).

## Fresh /tmp evidence (read at 13:16–13:17, supersedes iteration 23's)
- **Last interactive write anywhere: still 12:54:09** — `lh_old.err` 12:54:08.684, `lh_new.err` 12:52:31.340. The `lh_{old,new}.iterNN.txt` / `lh_diff.iterNN.txt` entries (13:10–13:14) are **driver iterations' own scratch** (layer-hash recounts), not interactive writes. This iteration's recount was in /tmp scratch as before (`lh_*.iter24.txt`, no writes to the WIP).
- **Layer-hash dump state, re-counted this iteration (regex-extract `LH run=(\d+) (mix|mlp) layer=(\d+) h=([0-9a-f]+)` from `lh_old.err` and `lh_new.err`, join in file order):** both dumps 1152 tuples; **1088 differing, 64 matching; first differing tuple: 0-indexed position 8 = run=150 mix layer=4** (diff hunk `9,128c9,128`; layers 0–3 of run 150 match for both mix and mlp; old h=`478a57cad0f3a017` vs new h=`4a44cf19f097a851`). Unchanged since iteration 15. The WIP FP8 fix was **not verified clean** at completion, unchanged since 12:54:09.

## Budget note
Driver budget is 30 h wall clock from ~09:00 UTC (2026-08-18) → clean exit around ~15:00 UTC (~1 h 43 min remaining at 13:17). If the interactive sessions stay alive, remaining iterations continue to back off in docs-only mode (that is by design; the driver stops cleanly on budget, and `supervisorctl start autoninfer-driver` re-arms it if the user wants more time).

## Serve state (do not touch)
- The live harness serve is the **manual** process (**pid 21537, up 04:56 at 13:16**) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED (since 08:30); `ninfer-serve-standby` STOPPED (since 08:44). `/v1/models` → 200 at 13:16.
- `/tmp/autoninfer-ops/pending.json` still holds `{"action":"restart-primary"}` (queued 08:45, deferred while interactive sessions are alive; re-verified 13:16). Expect the **supervisor** serve (wrapper flags incl. `--pending-timeout-ms 300000`) to come up once no interactive session is alive. Verify at start: `supervisorctl status ninfer-serve` + `curl -s http://127.0.0.1:8080/v1/models`. Do NOT re-queue it.
- GPU 1 idle (0% util). No GPU work ran this iteration (backoff), so no fresh `tools/gpu_health.sh 1` stamp.
...
1. **Check whether the interactive sessions are alive:** `ps -o pid,etime,cputime,stat -p 21662,237820` (plus `ps aux | grep -w "pi"` — exclude the driver's own session via the parent chain: its grandparent is `drive.sh` under `supervisord`), then `git status --short`, `git log --oneline -3`, and `git diff | sha256sum` (expect `d49b4f84…bb705` if the WIP is untouched).
   - **Any interactive pi alive with a dirty tree → back off again exactly as iterations 8–24 did** (refresh this handover from fresh `/tmp` evidence — last-write scan `ls -lat /tmp | head -15`, `stat` the WIP files, re-count layer-hash diffs: extract `grep -oE 'LH run=[0-9]+ (mix|mlp) layer=[0-9]+ h=[0-9a-f]+' lh_old.err` / same for `lh_new.err`, join in file order, diff — 1088/64 with first diff at 0-indexed 8 (line 9, hunk `9,128c9,128`) = run=150 mix layer=4 is the current expectation — commit ONLY the handover + BLOCKERS state if it moved, push, end). Do not touch the tree. Remember: driver scratch files in /tmp (e.g. `lh_*_keys.txt`, `lh_diff.iterNN.txt`, `lh_*.iterNN.txt`) are not interactive writes; check mtimes against 12:54:09.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:17: 1088/1152 layer-hash tuples still differ, first at line 9 = run=150 mix layer=4; no new interactive writes since 12:54:09).
...
- GPU 1: `bash tools/gpu_health.sh 1` before any benchmark. No GPU work ran this iteration (backoff #17).
