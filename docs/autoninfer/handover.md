...
**Last updated:** 2026-08-18 13:16 UTC by unattended driver session (iteration 22) — **backed off (15th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `64701ec9` (iteration 21's backoff docs commit); this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:14):
  - **pid 21662** (pts/6, up 04:51:39, 27:21 CPU — flat, idle) and **pid 237820** (pts/5, up 19:18, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:14) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–22; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:14: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09.132, `fp8_linear_add_small_t.cu`/`fp8_linear_swiglu_plan.cpp` 12:54:09.136, `test_fp8_linear_t_parity.cpp` 12:06:23, `test_tmp_binary_diff.cpp` 12:47:39).

## Fresh /tmp evidence (read at 13:14–13:15, supersedes iteration 21's)
- **Last interactive write anywhere: still 12:54** — `lh_old.err` 12:54, `lh_new.err` 12:52 (earlier scan). The `lh_{old,new}.iterNN.txt` / `lh_diff.iterNN.txt` / `lh_*_keys.txt` entries (13:07–13:13) are **driver iterations' own scratch** (layer-hash recounts), not interactive writes. This iteration's own scratch: `lh_{old,new}.iter22.txt`, `lh_diff.iter22.txt` (13:15).
- **Layer-hash dump state, re-counted this iteration (regex-extract `LH run=(\d+) (mix|mlp) layer=(\d+) h=([0-9a-f]+)` from `lh_old.err` and `lh_new.err`, join in file order):** both dumps 1152 tuples; **1088 differing, 64 matching; first differing tuple: 0-indexed position 8 = run=150 mix layer=4** (diff hunk `9,128c9,128`; layers 0–3 of run 150 match for both mix and mlp; old h=`478a57cad0f3a017` vs new h=`4a44cf19f097a851`). Unchanged from iteration 21. The WIP FP8 fix was **not verified clean** at completion, unchanged since 12:54:09.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:15: 1088/1152 layer-hash tuples still differ, first at line 9 = run=150 mix layer=4; no new interactive writes since 12:54:09).
...
- GPU 1: `bash tools/gpu_health.sh 1` before any benchmark. No GPU work ran this iteration (backoff #15).
