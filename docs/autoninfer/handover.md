...
**Last updated:** 2026-08-18 ~13:04 UTC by unattended driver session `autoninfer-driver-13` — **backed off (6th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `67a5b585` (iteration 12's backoff docs commit on top of `5814e90a`), pushed.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:02–13:03):
  - **pid 21662** (pts/6, up 04:40:04, 27:21 CPU — unchanged since the 13:00 sample of iter 11, i.e. idle) and **pid 237820** (pts/5, started 12:55:16, 0:02 CPU — idle) are both alive.
  - Both bash shells (3785 on pts/6, 11899 on pts/5) are children of **`/root/.local/bin/herdr server` (pid 3505)** — the ttys are daemon-held and **survive user disconnect**, so "abandoned terminal" is not a safe assumption. The pids staying alive IS the signal; don't wait for process death.
  - **WIP diff fingerprint (13:02) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 files, +105/−41); `tests/ops/test_fp8_linear_t_parity.cpp` sha256 = `92816bda…35748c` (mtime 12:06); `tests/ops/test_tmp_binary_diff.cpp` sha256 = `896bf98c…e12c13` (mtime 12:47). Identical to the 13:05 sample of iter 12 — **nothing in the tree has changed since 12:54:09**.
...
## Fresh /tmp evidence (read at 13:02–13:03, supersedes iteration 12's)
- **Last interactive write anywhere: still 12:54:09** — `lh_old.err`/`lh_old.out` 12:54, `lh_new.err`/`lh_new.out` 12:52, `opdump_*.txt` 12:47–12:49, `m_new_*.err` 12:46, `ninfer_new2` 12:46, `matrix.sh` 12:42, `ninfer_old` 12:42. Nothing in the tree or /tmp has changed since (checked 13:01–13:05 iter 12, 13:02–13:03 iter 13; tree mtimes 12:54:09, `text_context_impl.h` 12:50:36).
- `lh_old.err` (12:54:08) / `lh_new.err` (12:52:31) (NINFER_DEBUG_LAYERHASH AIME dumps): runs **completed** (1143 `LH` lines each, run=150..158). **Still diverging:** ~1095 differing layer hashes from layer 4 (mix+mlp) onward. The WIP FP8 fix was **not verified clean** at completion.

## Serve state (do not touch)
- The live harness serve is the **manual** process (pid 21537, started 08:20) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED; the manual process powers the harness (verified 13:02: `/v1/models` → 200; `nvidia-smi`: only pid 21537 on GPU 0, 30.5 GiB, 100% util during this session's own turns).
- `/tmp/autoninfer-ops/pending.json` still holds `{"action":"restart-primary"}` (queued 08:45, deferred while interactive sessions are alive; re-verified 13:03). Expect the **supervisor** serve (wrapper flags incl. `--pending-timeout-ms 300000`) to come up once no interactive session is alive. Verify at start: `supervisorctl status ninfer-serve` + `curl -s http://127.0.0.1:8080/v1/models`. Do NOT re-queue it.
- GPU 1 idle (0%, 4 MiB) at 13:03. Last HEALTHY stamp 12:57:22 (triad 1466 GiB/s, FMA 110.7 TFLOP/s); re-verify before any benchmark per protocol. No GPU work ran this iteration (backoff).
...
1. **Check whether the interactive sessions are alive:** `ps -o pid,etime,cputime -p 21662,237820` (plus `ps aux | grep -w " pi"` for any new one), then `git status --short`, `git log --oneline -3`, and `git diff | sha256sum` (expect `d49b4f84…bb705` if the WIP is untouched).
   - **Any interactive pi alive with a dirty tree → back off again exactly as iterations 8–13 did** (refresh this handover from fresh `/tmp` evidence — last-write scan `ls -lat /tmp | head`, `stat` the WIP files — commit ONLY the handover + BLOCKERS state if it moved, push, end). Do not touch the tree.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:03: finished dumps still show ~1095 layer-hash diffs; no new writes since 12:54:09).
