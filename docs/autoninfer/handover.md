...
**Last updated:** 2026-08-18 13:12 UTC by unattended driver session (iteration 19) — **backed off (12th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `f6817f68` (iteration 18's backoff docs commit); this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:11):
  - **pid 21662** (pts/6, up 04:48:15, 27:21 CPU — flat, idle) and **pid 237820** (pts/5, up 15:54, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:11) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–19; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:11: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09, `test_fp8_linear_t_parity.cpp` 12:06, `test_tmp_binary_diff.cpp` 12:47).

## Fresh /tmp evidence (read at 13:11, supersedes iteration 18's)
- **Last interactive write anywhere: still 12:54:08/09** — `lh_old.err`/`lh_old.out` 12:54, `lh_new.err`/`lh_new.out` 12:52. The `lh_{old,new}.iter17.txt` (13:08) / `lh_{old,new}.iter18.txt` (13:10) and `lh_{old,new}_keys.txt` (13:07) are **driver iterations' own scratch** (layer-hash recounts), not interactive writes. This iteration's own scratch: `lh_{old,new}.iter19.txt` / `lh_diff.iter19.txt` (13:11).
- **Layer-hash dump state, re-counted this iteration (file-order join of `grep -oE 'LH run=[0-9]+ (mix|mlp) layer=[0-9]+ h=[0-9a-f]+' lh_old.err` vs `lh_new.err`):** both dumps 1152 tuples; **1088 differing, 64 matching; first differing tuple: 9th line = 0-indexed position 8 = run=150 mix layer=4** (layers 0–3 of run 150 match for both mix and mlp; old h=`478a57cad0f3a017` vs new h=`4a44cf19f097a851`). Unchanged from iteration 18. The WIP FP8 fix was **not verified clean** at completion, unchanged since 12:54:09.
- **Process note (iter 17, still valid):** the `pi` processes with grandparent `drive.sh`/`timeout` (this iteration: 240790 under `timeout -k 60 3600 pi -p … autoninfer-driver-19`) are the **driver's own sessions** — exclude driver processes via the parent chain, not just the cmdline text.

## Budget note
Driver budget is 30 h wall clock from ~09:00 UTC (2026-08-18) → clean exit around ~15:00 UTC. If the interactive sessions stay alive, remaining iterations continue to back off in docs-only mode (that is by design; the driver stops cleanly on budget, and `supervisorctl start autoninfer-driver` re-arms it if the user wants more time).
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:11: 1088/1152 layer-hash tuples still differ, first at 9th line = run=150 mix layer=4; no new interactive writes since 12:54:08/09).
...
- GPU 1: `bash tools/gpu_health.sh 1` before any benchmark. No GPU work ran this iteration (backoff #12).
