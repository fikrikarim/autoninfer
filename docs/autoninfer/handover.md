...
**Last updated:** 2026-08-18 13:18 UTC by unattended driver session (iteration 25) — **backed off (18th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `014735a7` (iteration 24's backoff docs commit); this iteration adds a backoff docs commit on top.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:18):
  - **pid 21662** (pts/6, up 04:54, 27:21 CPU — flat, idle) and **pid 237820** (pts/5, up 22:19, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:18) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 modified + 2 new test files, +105/−41) — **byte-identical to iterations 11–25; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:18: `text_context_impl.h` 12:50:36, `fp8_config.h`/`fp8_dispatch.cpp` 12:54:09.132, `fp8_linear_add_small_t.cu`/`fp8_linear_swiglu_plan.cpp` 12:54:09.136, `test_fp8_linear_t_parity.cpp` 12:06:23, `test_tmp_binary_diff.cpp` 12:47:39).

## Fresh /tmp evidence (read at 13:18, supersedes iteration 24's)
- **Last interactive write anywhere: still 12:54:09** — `lh_old.err` 12:54:08.684, `lh_new.err` 12:52:31.340. The `lh_*.iter24.txt` entries (13:16) are iteration 24's own scratch (layer-hash recount), not interactive writes. This iteration's recount was in /tmp scratch as before (`lh_*.iter25.txt`, no writes to the WIP).
...
## Budget note
Driver budget is 30 h wall clock from ~09:00 UTC (2026-08-18) → clean exit around ~15:00 UTC (~1 h 42 min remaining at 13:18). If the interactive sessions stay alive, remaining iterations continue to back off in docs-only mode (that is by design; the driver stops cleanly on budget, and `supervisorctl start autoninfer-driver` re-arms it if the user wants more time).

## Serve state (do not touch)
- The live harness serve is the **manual** process (**pid 21537, up 04:57 at 13:18**) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED (since 08:30); `ninfer-serve-standby` STOPPED (since 08:44). `/v1/models` → 200 at 13:18.
- `/tmp/autoninfer-ops/pending.json` still holds `{"action":"restart-primary"}` (queued 08:45, deferred while interactive sessions are alive; re-verified 13:18). Expect the **supervisor** serve (wrapper flags incl. `--pending-timeout-ms 300000`) to come up once no interactive session is alive. Verify at start: `supervisorctl status ninfer-serve` + `curl -s http://127.0.0.1:8080/v1/models`. Do NOT re-queue it.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:18: 1088/1152 layer-hash tuples still differ, first at line 9 = run=150 mix layer=4; no new interactive writes since 12:54:09).
...
- GPU 1: `bash tools/gpu_health.sh 1` before any benchmark. No GPU work ran this iteration (backoff #18).
