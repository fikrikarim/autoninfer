...
**Last updated:** 2026-08-18 ~13:05 UTC by unattended driver session `autoninfer-driver-14` — **backed off (7th consecutive), no experiment this iteration** (interactive-session collision rule). State below.

## State
- **HEAD:** `220fe2e3` (iteration 13's backoff docs commit on top of `67a5b585`), pushed.
...
- **Working tree is DIRTY — still owned by live interactive pi sessions.** At this iteration's check (13:04):
  - **pid 21662** (pts/6, up 04:41, 27:21 CPU — unchanged since 13:02, i.e. idle) and **pid 237820** (pts/5, started 12:55:16, 0:02 CPU — idle) are both alive.
  - Parent chain still `herdr server` (pid 3505) — ttys are daemon-held and survive user disconnect; pids staying alive is the signal.
  - **WIP diff fingerprint (13:04) — compare before touching anything:** `git diff | sha256sum` = `d49b4f84…bb705` (8 files, +105/−41) — **byte-identical to iterations 11–13; nothing in the tree has changed since 12:54:09** (WIP file mtimes re-verified 13:04: `text_context_impl.h` 12:50:36, `test_tmp_binary_diff.cpp` 12:47, `test_fp8_linear_t_parity.cpp` 12:06).
...
## Fresh /tmp evidence (read at 13:04, supersedes iteration 13's)
- **Last interactive write anywhere: still 12:54:09** — `lh_old.err`/`lh_old.out` 12:54:08, `lh_new.err`/`lh_new.out` 12:52:31, `opdump_*.txt` 12:47–12:49, `m_new_*.err` 12:46, `ninfer_new2` 12:46, `matrix.sh` 12:42.
- **Layer-hash dump state, re-measured this iteration with a correct key join** (format is `LH run=N mix|mlp layer=M h=hash`, embedded mid-line in the .err files; key = (run, part, layer)): each dump has **1152 tuples (runs 150–158)**; old-vs-new join: **1088 differing, 64 matching** (earlier handovers said ~1095 — same finding). **First differing tuple: run=150 mix layer=10** (layers 0–9 of run 150 match). The WIP FP8 fix was **not verified clean** at completion, unchanged since 12:54:09.

## Serve state (do not touch)
- The live harness serve is the **manual** process (**pid 21537, started 08:20, verified 13:04**) on GPU 0: `ninfer-serve models/qwen3_8_27b_nvfp4.ninfer --host 127.0.0.1 --port 8080 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft` (lacks `--pending-timeout-ms 300000`). Supervisor `ninfer-serve` is STOPPED. `/v1/models` → 200 at 13:04; GPU 0 shows only this app (30.5 GiB).
- `/tmp/autoninfer-ops/pending.json` still holds `{"action":"restart-primary"}` (queued 08:45, deferred while interactive sessions are alive; re-verified 13:04). Expect the **supervisor** serve (wrapper flags incl. `--pending-timeout-ms 300000`) to come up once no interactive session is alive. Verify at start: `supervisorctl status ninfer-serve` + `curl -s http://127.0.0.1:8080/v1/models`. Do NOT re-queue it.
- GPU 1 idle (0%, 4 MiB) at 13:04. No GPU work ran this iteration (backoff), so no fresh `tools/gpu_health.sh 1` stamp.
...
1. **Check whether the interactive sessions are alive:** `ps -o pid,etime,cputime -p 21662,237820` (plus `ps aux | grep -w " pi"` for any new one), then `git status --short`, `git log --oneline -3`, and `git diff | sha256sum` (expect `d49b4f84…bb705` if the WIP is untouched).
   - **Any interactive pi alive with a dirty tree → back off again exactly as iterations 8–14 did** (refresh this handover from fresh `/tmp` evidence — last-write scan `ls -lat /tmp | head -15`, `stat` the WIP files, re-count layer-hash diffs with the key join noted above — commit ONLY the handover + BLOCKERS state if it moved, push, end). Do not touch the tree.
...
- The k=1 flip attribution — owned by the interactive sessions (FP8 linear small-t T=2..4 association + A16 island). Their evidence is in `/tmp` as listed above; their WIP was **not verified clean at completion** (13:04: 1088/1152 layer-hash tuples still differ; no new writes since 12:54:09).
