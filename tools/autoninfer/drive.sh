...
- Do exactly ONE experiment per iteration: pick the highest-value hypothesis from the backlog (or the next step in the handover), implement it, build with `cmake --build build -j`, measure with the protocol menu (M1 at minimum, plus the op-level check the change touches), compare against the recorded baseline, decide keep/discard, and append one row to docs/autoninfer/results.tsv (4 columns: commit, metric, status, and a ONE-LINE description of ~200 chars or less; the full attribution narrative goes in the commit message + handover.md, never the TSV cell).
- Quality policy (user rule, 2026-08-18): the north star is MAXIMUM speed; a SMALL measured quality degradation is acceptable when it buys a LARGE speedup (the user explicitly accepts e.g. BF16->NVFP4-style tradeoffs). Rules: (1) never assume - quantify with the quality gate: run `tools/autoninfer/quality_gate.sh pre-<exp>` BEFORE the change and `tools/autoninfer/quality_gate.sh post-<exp>` after, then diff the per-prompt token hashes (8 fixed prompts, greedy, MTP path). (2) Pure perf/schedule changes that cannot alter numerics must be token-IDENTICAL (zero hash diff) - any diff means numerics changed, so rule 3 applies. (3) Numerics-changing change: keep only if the M1 speedup is >=5% AND the quality diff is small (<=2 of 8 prompts changed and the changed outputs are still clearly correct); record the exact diff in the results.tsv description. (4) Large degradation (>=3 of 8 prompts changed, or an obvious behavioral collapse): discard, unless the speedup is large (>20%) - then keep the measurement, log it, and add a row to the Active blockers table in BLOCKERS.md asking the user to ratify the tradeoff. (5) Anything touching the external OpenAI/Anthropic protocol still needs the schema/parity checks; the quality gate covers token quality only.
- Web research (EXA API; key in the git-ignored .env): when choosing a new hypothesis, when stuck, or when a measurement is surprising, you may run up to 2 searches per iteration: `tools/autoninfer/exa_search.sh "<query>" [n]`, and read promising hits with `tools/autoninfer/exa_content.sh <url>` (papers, engineering blogs, kernel libraries are fair game). docs/autoninfer/inspiration.md already curates findings by hypothesis - read it before searching and append genuinely new actionable findings there (dedupe; one entry: source URL + concrete NInfer idea). Keep research to <=10 minutes per iteration; an idea is worth more than a survey.
